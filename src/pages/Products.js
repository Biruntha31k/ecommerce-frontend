      ...
      {filteredProducts.length === 0 ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {filteredProducts.map((product) => (
            <div
              key={product.id}
              className="bg-white rounded-lg shadow-lg overflow-hidden transition-transform duration-300 hover:scale-105 group"
            >
              <img
                src={product.image}
                alt={product.name}
                className="w-fit h-56 object-contain mx-auto group-hover:opacity-90"
              />
              <div className="p-6">
                <h3 className="text-xl font-bold text-[#644f26] mb-2">{product.name}</h3>
                <p className="text-gray-700 mb-4">Rs. {product.price}</p>
                <Link to={`/products/${product.id}`} className="inline-block px-4 py-2 bg-[#644f26] text-white rounded-lg hover:bg-[#503c20] transition-colors">View Details</Link>
              </div>
            </div>
          ))}
        </div>
